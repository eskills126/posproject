...
<div class="container-fluid">
	...
	<div class="col-xs-6 col-sm-8 col-md-offset-4">
		<h2 class="page-header text-center"><i class="fa fa-edit"></i>Product/Item Type</h2>
		</div>
...
<form id="frm" action="" method="post">
  ...
  <!-------------------------Form elements------------------------------------>    
        ...
        <label for="customer_id">ID:</label>
      	...
      	<div class="col-75">
        <input type="text" id="stid" name="stid" class="form-control" value="" readonly="" required>
      	</div>
    	</div>
		</div>
    	...
    	<div class="row">
      	<div class="col-25">
        <label for="itname">Item Type Name:</label>
      	</div>
      	<span class="asterisk_input"></span>
      	<div class="col-75">
        <input type="text" id="itname" name="itname" placeholder="Enter Item Type Name" required>
      	</div>
    	</div>
		</div>
...
<script>
$(document).ready(function(){
	$("#output").load("Masters_SalesItemType/view.php");
	$("#itname").focus();
	$("#save").click(function() {
		var id=$("#id").val();
		...
		$.ajax({
			 url:  "Masters_SalesItemType/insert.php",
			type:  "post",
			data: $("#frm").serialize(),
			success:function(d) {
				$("#output").load("Masters_SalesItemType/view.php");
		...
		$("#id").val("0");
			...
			$.ajax({
			url:  "Masters_SalesItemType/update.php",
			type:  "post",
			data: $("#frm").serialize(),
			success:function(d) {
	...
	$(document).on("click",".del",function(){
		...
		$.ajax({
			url:  "Masters_SalesItemType/delete.php",
			type:  "post",
			data: {id:id},
			success:function() {
				del.closest("tr").hide();
...
$(document).on("click",".edit",function(){
		...
		var stid = row.closest("tr").find("td:eq(0)").text();
			$("#stid").val(stid);
...
<script>
     ...
     $("#carea").autocomplete({
        source: "Masters_SalesItemType/autocomplete_area.php",
        minLength: 0,
...
<script>
	$(document).on('keypress',function(){
		if($("#id").val() == 0 ){
			$.ajax({
			 url:  "Masters_SalesItemType/maxid.php",
			type:  "post",
			data: $("#frm").serialize(),
			success:function(d) {
			$("#stid").val(d);
