...
<h4 class="page-header"><a class="btn btn-primary"><i class="fa fa-bars"></i></a>Customer Deatils :<hr/></h4>
	...
	<?php 
		$sql = "SELECT * FROM `salesitemgrouptbl` ";
		$res = $conn->query($sql);
		if ($res->num_rows>0) {
	...
	echo "<tr>";			
echo "<td>{$row["ProGroupId"]}</td>";
echo "<td>{$row["ProGroupTitle"]}</td>";
echo "<td><button type='button' class='btn btn-sm btn-info edit' data-id='{$row["ProGroupId"]}'><i class='fa fa-edit'></i></td>";
echo "<td><button type='button' class='btn btn-sm btn-danger del' data-id='{$row["ProGroupId"]}'><i class='fa fa-trash'></i></td>";
	echo "</tr>";
