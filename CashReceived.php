...
<form id="frm" action="" method="post">
  ...
  <!---------------------------------------------------------------->
<?php  
      $sql2 = "SELECT * FROM cashreceivetbl where cashrid=(SELECT max(cashrid) FROM cashreceivetbl) GROUP by cashrid";
...
<!------------------------------------------------------------------------------>
    <div class="form-group">
      <div class="row">
        ...
        <div class="col-25">
        <input type="text" name="cbal" id="cbal" placeholder="Cash Balance"style="width:80%;background-color:#edf1f7;" readonly>
...
$(document).ready(function(){
  ...
  $("#save").click(function() {
    ...
    $.ajax({
       url:  "Transaction_CashReceived/insert.php",
      type:  "post",
      data: $("#frm").serialize(),
      success:function(d) {
        $("#output").load("Transaction_CashReceived/view.php");
    ...
    $("#id").val("0");
      ...
      $.ajax({
      url:  "Transaction_CashReceived/update.php",
      type:  "post",
      data: $("#frm").serialize(),
      success:function(d) {
  ...
  $(document).on("click",".del",function(){
    ...
    $.ajax({
      url:  "Transaction_CashReceived/delete.php",
      type:  "post",
      data: {id:id},
      success:function() {
        del.closest("tr").hide();
...
<script>
   $(function() {
     ...
     $("#acode").autocomplete({
        source: "Transaction_CashReceived/autocomplete_area.php",
        minLength: 0,
  ...
  $(document).ready(function(){
    $("#acode").on('keyup',function(){
      var updatedname = $(this).val();
        ...
        $.ajax({
          type: 'POST',
          url: 'Transaction_CashReceived/namedynamicupdate.php',
