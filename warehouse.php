...
<div class="container-fluid">
	...
	<div class="col-xs-6 col-sm-8 col-md-offset-4">
		<h2 class="page-header text-center"><i class="fa fa-edit"></i>Warehouse Detail</h2>
		</div>
...
<form id="frm" action="" method="post">
  ...
  <!-------------------------Form elements------------------------------------>    
        ...
        <label for="customer_id">ID:</label>
      	...
      	<div class="col-75">
        <input type="text" id="wid" name="wid" class="form-control" value="" readonly="" required>
      	</div>
    	</div>
		</div>
        ...
        <label for="wname">Warehouse Name:</label>
      	...
      	<div class="col-75">
        <input type="text" id="wname" name="wname" placeholder="Enter Item Warehouse Name" required>
      	</div>
    	</div>
		</div>
...
<script>
$(document).ready(function(){
	$("#output").load("Masters_Warehouse/view.php");
	$("#wname").focus();
	$("#save").click(function() {
		var id=$("#id").val();
		//if ($("#cname").val()=="" || $("#copbal").val()=="" || $("#carea").val()=="" )
		if ($("#wname").val()=="" ){
			alert("Please Add Warehouse Name ");
			$("#wname").focus();

		}else if (id==0){
		$.ajax({
			 url:  "Masters_Warehouse/insert.php",
			type:  "post",
			data: $("#frm").serialize(),
			success:function(d) {
				$("#output").load("Masters_Warehouse/view.php");
		...
		$("#id").val("0");
			...
			$.ajax({
			url:  "Masters_Warehouse/update.php",
			type:  "post",
			data: $("#frm").serialize(),
			success:function(d) {
	...
	$(document).on("click",".del",function(){
		...
		$.ajax({
			url:  "Masters_Warehouse/delete.php",
			type:  "post",
			data: {id:id},
			success:function() {
				del.closest("tr").hide();
...
$(document).on("click",".edit",function(){
		...
		var wid = row.closest("tr").find("td:eq(0)").text();
			$("#wid").val(wid);
...
<script>
     ...
     $("#carea").autocomplete({
        source: "Masters_Warehouse/autocomplete_area.php",
        minLength: 0,
...
<script>
	$(document).on('keypress',function(){
		if($("#id").val() == 0 ){
			$.ajax({
			 url:  "Masters_Warehouse/maxid.php",
			type:  "post",
			data: $("#frm").serialize(),
			success:function(d) {
			$("#wid").val(d);
